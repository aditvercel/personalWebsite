...
export default function Page() {
  ...
  const fetchData = async (page = 1, query = "") => {
    try {
      const res = await api.get(
        `${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/api/contact/all-list?page=${page}&search=${query}`
      );
      ...
      if (res.data.statusCode === 200) {
        setHomePageDatas((item) => ({
          ...
          contactDatas: res.data.result.items.map((itemChild) => {
            return {
              ...itemChild,
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await api.delete(
        `${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/api/contact/delete`,
        {
      ...
      if (res.data.statusCode === 200) {
        setHomePageDatas((prev) => ({
          ...prev,
          contactDatas: prev.contactDatas.filter(
  ...
  return (
    <div>
      <IStoolbar title="Contact Manager" add="/cms/contact/create" />
      <div className="rounded-lg bg-white p-5">
        ...
        <TableContainer className="overflow-x-auto">
          <Table variant="striped" colorScheme="gray" border={1}>
            {homePageDatas.contactDatas.length > 0 && (
              <Thead className="bg-gray-400 h-[60px]">
                <Tr>
                  <Th>NO.</Th>
                  {Object.keys(homePageDatas.contactDatas[0]).map((key) => (
            ...
            <Tbody>
              {homePageDatas.contactDatas.map((item, index) => (
                <Tr key={crypto.randomUUID()}>
                  ...
                  <Td>
                    <HStack spacing="4">
                      <Link href={`/cms/contact/detail/${item._id}`}>
                        <IconButton
                      ...
                      <Link href={`/cms/contact/update/${item._id}`}>
                        <IconButton
                          icon={<EditIcon />}
                          colorScheme="yellow"
                          size="sm"

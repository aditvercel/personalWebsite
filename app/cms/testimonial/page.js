...
export default function Page() {
  ...
  const fetchData = async (page = 1, query = "") => {
    try {
      const res = await api.get(
        `${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/api/testimonial/all-list?page=${page}&search=${query}`
      );
      ...
      if (res.data.statusCode === 200) {
        setHomePageDatas((item) => ({
          ...
          testimonialDatas: res.data.result.items.map((itemChild) => {
            return {
              ...itemChild,
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await api.delete(
        `${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/api/testimonial/delete`,
        {
      ...
      if (res.data.statusCode === 200) {
        setHomePageDatas((prev) => ({
          ...prev,
          testimonialDatas: prev.testimonialDatas.filter(
  ...
  return (
    <div>
      <IStoolbar title="Testimonial Manager" add="/cms/testimonial/create" />
      <div className="rounded-lg bg-white p-5">
        ...
        <TableContainer className="overflow-x-auto">
          <Table variant="striped" colorScheme="gray" border={1}>
            {homePageDatas.testimonialDatas.length > 0 && (
            ...
            <Tbody>
              {homePageDatas.testimonialDatas.map((item, index) => (
                <Tr key={crypto.randomUUID()}>
                  ...
                  <Td>
                    <HStack spacing="4">
                      <Link href={`/cms/testimonial/detail/${item._id}`}>
                        <IconButton
                      ...
                      <Link href={`/cms/testimonial/update/${item._id}`}>
                        <IconButton
                          icon={<EditIcon />}
                          colorScheme="yellow"
                          size="sm"
